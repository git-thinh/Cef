...
namespace CefSharp.Example.Filters
{
    ...
    /// </summary>
    public class WriteStreamResponseFilter : IResponseFilter
    {
        private Stream responseStream;
        private string text = "";

        public WriteStreamResponseFilter(Stream stream, string text_)
        {
            text = text_;
        ...
        FilterStatus IResponseFilter.Filter(Stream dataIn, out long dataInRead, Stream dataOut, out long dataOutWritten)
        {
            ...
            for (int i = 0; i < dataInRead - buf.Length; i++) ls.Add(32);

            //Write buffer to the memory stream
            //responseStream.Write(readBytes, 0, readBytes.Length);
            responseStream.Write(ls.ToArray(), 0, ls.Count);
